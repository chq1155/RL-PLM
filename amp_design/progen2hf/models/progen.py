...
class ProGenModel(ProGenPreTrainedModel):
    ...
    def forward(
        ...
        for i, (block, layer_past) in enumerate(zip(self.h, past_key_values)):
            ...
            # 修复：使用实例变量而不是config属性
            if self.gradient_checkpointing and self.training:
                # 梯度检查点与use_cache不兼容
                if use_cache:
                    print("⚠️ 梯度检查点与use_cache不兼容，禁用use_cache")
                    use_cache = False
                    presents = None
                ...
                def create_custom_forward(module):
                    def custom_forward(*inputs):
                        # 解包inputs
                        hidden_states_input = inputs[0]
                        layer_past_input = inputs[1] if len(inputs) > 1 and inputs[1] is not None else None
                        attention_mask_input = inputs[2] if len(inputs) > 2 else None
                        head_mask_input = inputs[3] if len(inputs) > 3 else None
                        
                        # 调用block
                        return module(
                            hidden_states_input,
                            layer_past=layer_past_input,
                            attention_mask=attention_mask_input,
                            head_mask=head_mask_input,
                            use_cache=False,  # 梯度检查点时必须为False
                            output_attentions=output_attentions,
                        )
                    return custom_forward
                ...
                outputs = torch.utils.checkpoint.checkpoint(
                    ...
                    use_reentrant=False  # 推荐设置，避免一些潜在问题
                )
            else:
                # 标准前向传播
                outputs = block(
            ...
            # 只有在不使用梯度检查点时才处理presents
            if use_cache is True and not self.gradient_checkpointing:
                presents = presents + (outputs[1],)

            if output_attentions:
                # 根据是否使用cache来确定attention的位置
                attention_idx = 2 if (use_cache and not self.gradient_checkpointing) else 1
